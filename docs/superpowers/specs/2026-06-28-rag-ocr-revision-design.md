...
对比 GraphRAG / LinearRAG / DeepSearcher 后沉淀的判断，登记于此以免丢失。
**本次实现仍只聚焦 A/B/C**；以下为远期、按需推进。

### 多跳/复杂推理：两条路，优先走"agent 迭代"

复杂法律问答（如"甲签约时是否有代理权"需串 合同→签约人→授权书→时间线）有两条解法：

- **(b) agent 迭代（DeepSearcher 思路，近期、低风险、首选）**：不建任何图，靠
  lawiki agent **多轮顺序检索 + 反思**逼近多跳——本就落在我们"前半段 RAG +
  agent 拥有 LLM"的架构里，几乎白得。
  - 落地形态：一个 lawiki **"深度研究"工作流（指令层，非代码）**：
    复杂问题 → 拆子问题 → 多轮 wiki+RAG 双路检索 → 每轮反思是否够答/是否需新一跳
    → 收口走逐字锚点 + wiki×RAG 交叉校验 → 给带溯源答案；简单问题直接单轮。
  - 风险：多轮误差累积——必须靠反思 + wiki×RAG 校验兜底。
- **(a) 图索引（LinearRAG 思路，远期、可选）**：实体—段落图 + 语义桥接做单趟多跳。
  零 LLM 建图、确定性、与"可校验"哲学契合，但需先解决前置门槛（见下）。
  因 (b) 已能吃下多数多跳，**(a) 降为远期可选优化**。

### GraphRAG —— 不采纳为真相源

LLM 自动抽关系建图，但三元组**幻觉、不可验证**，与 wiki 的逐字溯源对立。
最多作 RAG 侧的关系召回增强或 wiki ingest 的初稿线索（标 INFERRED 再过验证闸门），
**绝不替代 wiki 的验证职责**。LinearRAG 是其"零 LLM、确定性"的更优替代。

### 工作流 D（远期可选）：LinearRAG 式实体桥接多跳召回

- **准入门槛（必须先过）**：一颗 time-boxed **"中文法律实体层可信度"spike**，
  用真实案件 `_md` 验证：混合 NER（统计模型认软实体 + 正则认强模式实体
  案号/法条/金额/日期）+ **显式简称共指**（"以下简称…"，确定性可机检）能否得到
  足够可信的实体层。量两个数：强模式实体准确率、共指**错并率**（法律最怕错并→假跳）。
- spike 过才值得投入移植 LinearRAG（换 NER/embedding、改检索、补 provenance）；
  不过则继续观望。排在 A/B/C 与 (b) 之后。
