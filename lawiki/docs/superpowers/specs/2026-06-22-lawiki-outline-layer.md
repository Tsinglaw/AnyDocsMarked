...
对抗"similarity ≠ relevance"的三类做法,本项目的取舍:

- **结构导航(outline)** —— 已采纳(本设计)。零依赖、离线、亦作降级。
- **精确词检索** —— 用 **grep `_md`** 解决,作为 qa.md 第四条取证路(法条号/姓名/金额/案号);**刻意不引入 BM25/FTS 引擎**:案件规模(几千 chunk)用不上排序/融合,中文精确子串 grep 已够,且零依赖。
- **换嵌入式向量库(如 alibaba/zvec:向量+FTS+标量过滤的混合检索、DiskANN、十亿级)** —— **暂不采纳**:规模与原生混合检索对当前用例过剩,且会引入 C++ 新依赖、重写 `store.py`。但因 lawiki ↔ rag-retriever 是 **CLI/JSON 契约解耦,向量后端可无感替换**,若将来确需规模化/生产硬化/原生混合检索,zvec 是替换 LanceDB 的强候选,届时 lawiki 不受影响。
